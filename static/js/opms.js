$(function(){
	...
	$('.js-knowledgessort-single').on('click', function(){
    	...
    	var status = that.attr('data-status')
    	var id = that.attr('data-id');
		$.post('/knowledgessort/ajax/status', { status: status, id: id },function(data){
			dialogInfo(data.message)
			if (data.code) {
				that.attr('data-status', status == 2 ? 1 : 2).text(status == 2 ? '正常' : '屏蔽').parents('td').prev('td').text(status == 2 ? '屏蔽' : '正常');
			} else {
	...
	$('.js-knowledgessort-delete').on('click', function(){
    	var that = $(this);
    	var id = that.attr('data-id');
		$.post('/knowledgessort/ajax/delete', { id: id },function(data){
			dialogInfo(data.message)
			if (data.code) {
        ...
        submitHandler:function(form) {
            $(form).ajaxSubmit({
                ...
                success:function(data) {
                    dialogInfo(data.message)
                    if (data.code) {
                       setTimeout(function(){window.location.href="/knowledgessort/list"}, 2000);
                    } else {
                       setTimeout(function(){ $('#dialogInfo').modal('hide'); }, 1000);
                    }
                }
        ...
        submitHandler:function(form) {
            $(form).ajaxSubmit({
                ...
                success:function(data) {
                    dialogInfo(data.message)
                    if (data.code) {
						setTimeout(function(){window.location.href='/customer/list';}, 2000);
                    } else {
                        setTimeout(function(){ $('#dialogInfo').modal('hide'); }, 1000);
                    }					
                }
	...
	$('.js-customers-single').on('click', function(){
    	var that = $(this);
    	var status = that.attr('data-status')
    	var id = that.attr('data-id');
		$.post('/customer/ajax/status', { status: status, id: id },function(data){
			dialogInfo(data.message)
			if (data.code) {
				that.parents('td').prev('td').text(that.text());
